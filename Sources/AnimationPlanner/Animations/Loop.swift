...
public struct Loop<Looped> {
    ...
    public let loopedAnimations: [Looped]
    
    fileprivate init(
        repeatCount: Int,
        @SequenceBuilder builder: (_ index: Int) -> [Looped]
    ) {
        self.loopedAnimations = (0..<repeatCount).flatMap(builder)
        
        if Looped.self == SequenceAnimatable.self, let animations = loopedAnimations as? [SequenceAnimatable] {
            duration = animations.reduce(0, { $0 + $1.duration })
        } else if Looped.self == GroupAnimatable.self, let animations = loopedAnimations as? [GroupAnimatable] {
    ...
    fileprivate static func map<S: Swift.Sequence>(
        _ sequence: S,
        @SequenceBuilder with builder: (S.Element) -> [Looped]
    ) -> [Looped] {
...
extension Loop: GroupConvertible where Looped == GroupAnimatable {
    ...
    public init(
        for repeatCount: Int,
        @GroupBuilder animations builder: (_ index: Int) -> [GroupAnimatable]
    ) {
        self.init(repeatCount: repeatCount, builder: builder)
    }
    ...
    public static func through<S: Swift.Sequence>(
        _ sequence: S,
        @GroupBuilder animations builder: (S.Element) -> [GroupAnimatable]
    ) -> [any GroupAnimatable] {
        map(sequence, with: builder)
    }
}
...
extension Loop: PerformsAnimations where Looped == GroupAnimatable {
    public func animate(delay leadingDelay: TimeInterval, completion: ((Bool) -> Void)?) {
        Group(animations: loopedAnimations).animate(delay: leadingDelay, completion: completion)
    }
}
